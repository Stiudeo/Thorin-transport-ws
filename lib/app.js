...
module.exports = function (thorin, SocketEvent) {
  ...
  class ThorinWsApp {
    ...
    addHandler(actionObj) {
      ...
      if (SOCKET_EVENT.__contains(actionObj.name)) {
        let roomEvent = SOCKET_EVENT.__isRoomEvent(actionObj.name);
        if (roomEvent !== false) {
          if (typeof this[socketEvents][roomEvent] === 'undefined') {
            this[socketEvents][roomEvent] = [];
          }
          this[socketEvents][roomEvent].push(actionObj);
          return this;
        }
      ...
      thorin.on(thorin.EVENT.RUN, 'transport.' + httpTransportObj.name, () => {
        ...
        getAppAdapter(thorin, this[config].adapter, (redisAdapter) => {
          thorin.on(thorin.EVENT.RUN, () => {
            ...
            if (!this[socketEvents][SOCKET_EVENT.CONNECT]) {
              logger('warn', `Websocket server has no authorization (${SOCKET_EVENT.CONNECT}) handler`);
            }
      ...
      this._onConnected(() => {
        ...
        if (eventObj._hasRooms()) {
          for (let i = 0, len = eventObj._rooms.length; i < len; i++) {
            ...
            try {
              app.to(roomName).emit(eventObj._event, data, roomName);
            } catch (e) {
  ...
  function handleClientAuthorization(socketObj, next) {
    ...
    let authData = this[transportApp]._getAuthorization(req, this[config].authorization);
    ...
    handleSocketEvent.call(this, SOCKET_EVENT.CONNECT, socketObj, (wasError, d) => {
      if (wasError) {
        let e = new Error(d.error.code);
        socketObj.disconnect(e);
        return next(e);
      }
      socketObj.thorinStartTime = Date.now();
      next();
    });
  ...
  function handleClientDisconnect(socketObj) {
    ...
    handleSocketEvent.call(this, SOCKET_EVENT.DISCONNECT, socketObj, () => {
      ...
      delete socketObj.thorinStartTime;
    });
  }
  ...
  function handleClientRoomEvent(socketObj, actionType, roomName, _payload, _fn) {
    ...
    let actionObj,
      roomNameHandler = (typeof roomName === 'string' && roomName ? actionType + ':' + roomName : null);
    if (roomNameHandler && typeof this[socketEvents][roomNameHandler] !== 'undefined') {
      actionType = roomNameHandler;
    }
    for (let i = 0; i < this[socketEvents][actionType].length; i++) {
      let a = this[socketEvents][actionType][i],
        roomInput = a.match(roomName);
      if (roomInput) {
        payload = thorin.util.extend(payload, roomInput);
        actionObj = a;
        break;
      }
    }
    ...
    /* CHECK if we have a room pattern in the action. If so, check it for matchers. */
    let roomPattern = actionObj.roomName();
    if (roomPattern) {
      let matchData = roomPattern.match(roomName);
      if (!matchData) {
        req.alias = roomName;
        return sendEventError.call(this, req, thorin.error('ROOM.UNAVAILABLE', 'The requested room is unavailable', 401));
      }
      req.payload = thorin.util.extend(payload, matchData);
    }
    const _opType = (actionType.indexOf(SOCKET_EVENT.ROOM_JOIN) === 0 ? 'join' : 'leave'),
    ...
    try {
      dispatchIntent.call(this, actionObj.name, req, (wasErr, data, intentObj) => {
        if (intentObj.room) {
